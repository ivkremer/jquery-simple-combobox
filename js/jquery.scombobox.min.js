...
y.change().data("changed",!0)}});this.on("focus",b+k,function(){if(!this.value.trim())a.data(c).expandOnFocus&&d(this).keyup();else if(a[c]("val")){var e=a.children(b+n);e.children().show();t.call(e,"down")}});this.on("click",b+k+"-div",function(){a.data(c).disabled||t.call(d(this).siblings(b+n),"down")});this.on("click",b+k,function(a){var e=this;d(b).each(function(){if(this!=e)d(this)[c]("close")});a.stopPropagation()});this.on("click",b+s,function(a){a.stopPropagation();a=d(this).closest(b);var e=
a.children(b+n);e.is(":visible")?t.call(e,"up"):(t.call(e,"down"),a.children(b+k).focus())});this.on("click",b+J,function(c){c.stopPropagation();c=d(this);var h=c.parent();a.children(b+n).children("p").eq(c.data("index")).find(":checkbox").prop("checked",!1);h.fadeOut(e.animation.duration);c.closest(b).children("select").trigger("change",[!0])});e.autoLoad!=d.noop&&d(b+n,this).scroll(function(b){b=d(this);var c=a.children("select"),f=b.scrollTop();f>b.data("scrollTop")?this.scrollHeight-f-50<b.height()&&
!a.data("pending")&&(a.data("pending",!0),e.autoLoad.call(a,c.find("option[value]:last").val(),"bottom")):f<b.height()/2&&!a.data("pending")&&(a.data("pending",!0),e.autoLoad.call(a,c.find("option[value]:first").val(),"top"));b.data("scrollTop",f)}).data("scrollTop",0);K||(K=!0,d(L).bind("click."+c,function(){t.call(d(b+n),"up")}));this.data("listenersAdded",!0)}}function Q(a){if("string"==typeof a&&(a=d.parseJSON(a),null==a))return[];if(!a)return!1;if(!(a instanceof Array)){if("object"!=typeof a)return!1;
"undefined"==typeof a.length&&(a.length=Object.keys(a).length);a=[].slice.call(a)}return a}function R(a,b){for(var c=0;c<a.length;c++)a[c].value&&a[c].text||a[c].hasOwnProperty("separator")||a.splice(c,1)}function S(a){for(var b=0;b<a.length;b++)a[b].value||d(a[b]).hasClass(c+x)||"optgroup"==a[b].tagName.toLowerCase()||d(a[b]).remove()}function T(a,b){return a.text.trim().toLowerCase()>b.text.trim().toLowerCase()?1:-1}function U(a){for(var b=0;b<a.length;b++)for(var c=b+1;c<a.length;c++)a[b]&&a[c]&&
a[b].value==a[c].value&&a.splice(b,1)}function V(a){for(var b=0;b<a.length;b++)for(var c=b+1;c<a.length;c++)a[b]&&a[c]&&a[b].value==a[c].value&&"optgroup"!=a[b].tagName.toLowerCase()&&d(a[b]).remove()}function t(a,e){if(!this.is(":animated")&&this.length&&("up"!=a||!this.is(":hidden")||1!=this.length)){var g=this.parent().data(c).animation;d.easing[g.easing]||(console.warn("no such easing: "+g.easing),g.easing="swing");var h=this.parent(),f=h.data(c);"up"==a?(f.beforeClose.call(h),g.complete=function(){f.afterClose.call(h)},
this.slideUp(g).data("p-clicked-index",-1),h.children(b+s).removeClass(c+s+"-up")):(f.beforeOpen.call(h),g.complete=function(){f.afterOpen.call(h)},this.slideDown(g),h.children(b+s).addClass(c+s+"-up"));h.children(b+k).each(function(){var a=d(this);a.data("fillonarrow")&&!e&&a.data("fillonarrow",!1).val(a.data("value"))})}}function I(a,e){var g=d(this),h=g.closest(b),f=g.parent(),l=f.children("p"),m=l.index(this),k=M(f.parent().data(c).animation.duration);if(!e){var p=g.find(":checkbox");d(a.target).is(":checkbox")||
p.prop("checked",!p.prop("checked"));p=p.prop("checked");if(a.shiftKey&&0<=f.data("p-clicked-index"))for(var n=f.data("p-clicked-index"),r=n<m?m:n,n=n<m?n:m;n<=r;n++)d(l[n]).find(":checkbox").prop("checked",p)}var q=h.find(b+D).prepend("<span />");h.find(b+D).fadeOut(k/5,function(){q.empty().show();l.each(function(a){var e=d(this);e.find(":checkbox").prop("checked")&&q.append(d("<div />").addClass(c+B).append(d("<div />").addClass(c+B+"-text").text(e.find(b+A).text())).append(d("<div />").addClass(c+
J).text("\u00d7").data("index",a)).fadeIn(1.5*k))});q.append('<div style="clear: both" />')});f.data("p-clicked-index",m);g.closest(b).children("select").trigger("change",[!0])}function W(a,e){for(var g=this.data(c),h=this.find("select"),f=this.find(b+n),l=0;l<a.length;l++){if(a[l].hasOwnProperty("separator"))var m=a[l].hasOwnProperty("header")?d('<p class="'+c+z+'" />').text(a[l].header):d('<p class="'+c+x+'" />'),k=d("<option />");else k=d("<option />").val(a[l].value).text(a[l].text),m=g.pFillFunc.call(this,
a[l],g),"checkboxes"==g.mode&&m.prepend('<input type="checkbox" />');m.data("value",a[l].value);e?(h.prepend(k),f.prepend(m)):(h.append(k),f.append(m))}}function H(a){var c=a.children(b+u+":visible");0==c.length&&(c=a.children(":visible:first"));return c}function G(a){if(null==a)return null;for(var b=Object.keys(a),c=0;c<b.length;c++){var d=b[c].replace(/-([a-z])/g,function(a){return a[1].toUpperCase()});b[c]!=d&&(a[d]=a[b[c]],delete a[b[c]]);"object"==typeof a[d]&&"data"!=d&&G(a[d])}return a}var c=
"scombobox",b="."+c,k="-display",r="-value",v="-invalid",C=k+"-div",B=C+"-item",J=B+"-remove",D=C+"-holder",n="-list",A="-mainspan",u="-hovered",x="-separator",z="-header",q="-dropdown-background",s="-dropdown-arrow",K=!1,E=parseInt,F={init:function(){var a=this.find("."+c+n),e=this.find("select"),g=this.find(b+q),h=this.find(b+s),f=this.data(c);this.addClass(c);0==e.length&&this.append(d("<select />"));this.attr("id")&&e.removeAttr("id");e.attr("multiple")&&(this.data(c).mode="checkboxes");0==g.length&&
this.append('<div class="'+c+q+'" />');0==h.length&&this.append('<div class="'+c+s+'" />');F.displayDropdown.call(this,f.showDropDown);"checkboxes"!=f.mode&&0==this.find(b+k).length&&(e=d('<input class="'+c+k+'" type="text" />'),this.append(e),this.height(+e.css("font-size")+ +e.css("padding-top")+ +e.css("padding-bottom")));null!=f.tabindex&&this.find(b+k).attr("tabindex",f.tabindex);0==this.find(b+r).length&&this.append('<input class="'+c+r+'" type="hidden" />');(this.find(b+k).is(":disabled")||
f.disabled)&&this.find(b+q+", "+b+s).hide();f.disabled&&(this.find(b+k).prop("disabled",!0),this.addClass(c+"-disabled"));0==a.length&&this.append(a=d('<div class="'+c+n+'"></div>'));"checkboxes"==f.mode?(this.addClass(c+"-checkboxes"),this.find(b+k).remove(),0==this.find(b+k+"-div").length&&this.append('<div class="'+c+C+'"><div class="'+c+D+'" /></div>'),a.insertAfter(this.find(b+k+"-div")),g=this.find(b+D),e=d('<div class="'+c+B+'" id="'+c+'-test-item"><div class="'+c+B+'-text">x</div></div>'),
g.append(e.css("margin-left","-9999px").show()),g=e.height()+2*E(e.css("padding-top"))+2*E(e.css("margin-top"))+2*E(e.css("border-top-width"))+2*E(g.css("padding-top")),this.find(b+k+"-div").css("min-height",g+"px"),e.remove()):(this.find(b+"-display-div").remove(),a.insertAfter(this.find(b+k)));a.css({"max-width":f.listMaxWidth,"max-height":f.maxHeight});!0==f.wrap&&a.css("white-space","normal");f.autoLoad!=d.noop&&(f.loopScrolling=!1);P.call(this);this.data(c+"-init",!0);return F.fill.call(this,
f.data)},fill:function(a,e){var g=this.find("select").children("option, optgroup"),h=this.find("."+c+n),f=this.find("select");a=Q(a);var l=this.data(c),m=l.mode;a?(l.removeDuplicates&&U(a),R(a),l.sort&&(a.sort(T),l.sortAsc||a.reverse()),e||(f.empty(),h.empty(),this.children(b+r+", "+b+k).val("")),W.call(this,a,2==e)):(l.removeDuplicates&&(V(g),S(g),g=this.find("select").children("option, optgroup")),0!=g.length&&g.each(function(){var a=d(this),b=d("<p />");if(a.hasClass(c+x))a.hasClass(c+z)?h.append(b.addClass(c+
z).text(a.text())):b.addClass(c+x);else{if("optgroup"==this.tagName.toLowerCase()){var e=a.attr("label"),f=d("option",this);a.before("<option />");a.after(f);a.remove();h.append(e?b.addClass(c+z).text(e):b.addClass(c+x));f.each(function(){h.append(d("<p />").text(d(this).text()).data("value",this.value))});return}b.append(d('<span class="'+c+A+'" />').text(a.text())).data("value",this.value);"checkboxes"==m&&b.prepend('<input type="checkbox" />')}h.append(b)}));this.data(c+"-init")&&(l.callback.func.apply(this,
l.callback.args),this.data(c+"-init",!1));if("checkboxes"!=m)this[c]("val",g.filter("option:selected:last").val());else g=g.filter(":selected").map(function(){return d(this).val()}).get(),this[c]("val",g);return this},clear:function(){this.children("select").empty();this.children(b+n).empty().width("");this.children(b+k).removeClass(c+v);this.children(b+q).removeClass(c+q+v);return this},data:function(a){if(0==arguments.length)return this.data(c).data;this.data(c).data=a;return this},disabled:function(a){var e=
this.data(c).mode;if(0==arguments.length)return"checkboxes"==e?this.hasClass(c+"-disabled"):this.children(b+k).prop("disabled");a=!!a;this.children(b+k).prop("disabled",a);a?(this.addClass(c+"-disabled"),this.children(b+q+", "+b+s).hide()):(this.removeClass(c+"-disabled"),this.children(b+q+", "+b+s).show());return this},tabindex:function(a){var c=this.find(b+k);if(0==arguments.length)return c.attr("tabindex");c.attr("tabindex",a);return this},options:function(a){if(0==arguments.length)return this.data(c);
